    ...
    private void ShowUpdateLog()
    {
        ModBase.RunInNewThread(() =>
        {
            ...
            if (ModMain.MyMsgBoxMarkdown(changelog,
                ...
                2) ModBase.OpenWebsite("https://github.com/PCL-In/desktop/releases");
        }, "UpdateLog Output");
    }
            ...
            try
            {
                ...
                if (Environment.GetEnvironmentVariable("PCL_DISABLE_DEBUG_HINT") is null)
                {
                    ...
                    ModMain.MyMsgBox(
                        ...
                        {
                            ModBase.OpenWebsite("https://github.com/PCL-In/desktop/releases/latest");
                            EndProgram(false);
                        });
                }
        ...
        bool force = true, bool isUpdating = false)
    {
        ...
        if (returnCode == ModBase.ProcessReturnValues.Exception)
        {
            if (!isLogShown)
            {
                ...
                ModBase.Log("请在 https://github.com/PCL-In/desktop/issues 提交错误报告，以便于社区解决此问题！（这也有可能是原版 PCL 的问题）");
                isLogShown = true;
